...
function unuspay_run_migration() {
	...
	dbDelta("
		CREATE TABLE {$wpdb->prefix}wc_unuspay_logs (
	...
	if ( class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) ) {
		add_action(
			...
			function( Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry ) {
				...
				if ( get_option( 'unuspay_wc_gateway_type' ) == 'multigateway' ) {

					$blockchains = json_decode( get_option( 'unuspay_wc_blockchains' ) );

					foreach ($blockchains as $blockchain) {
						require_once UNUSPAY_WC_ABSPATH . 'includes/class-unuspay-wc-payments-block-' . $blockchain . '.php';
						$className = 'UnusPay_WC_Payments_Block_' . ucfirst($blockchain);
